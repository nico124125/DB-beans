...
def prompt_see_all_planes(connection):
    ...
    for plane in planes:
        print(f"{plane[1]} ({plane[2]}) - {plane[3]}/100")
...
4) See which preparation method is best for a beans. 
5) delete bean
6) Exit.
...
def prompt_delete_bean(connection):
    ...
    delete = database.delete_bean_by_name(connection, name)
